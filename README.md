...
- To fix resolution issues on Windows do the following: `task manager>java process properties>compatibility tab>high dpi settings>ignore high dpi scaling>let system scale`. Alternatively set your screen resolution to 100% in system settings, or uncomment the `System.setProperty("sun.java2d.uiScale", "1.0");` in the main method (Game.java)
